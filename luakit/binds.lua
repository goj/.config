...
add_binds("normal", {
    ...
    key({"Control"}, "l",           function (w, c) w:enter_cmd(":open ") end),
    key({"Control", "Shift"}, "L",  function (w, c) w:enter_cmd(":open "    .. (w.view.uri or "")) end),
    key({"Control"}, "g",           function (w, c) w:enter_cmd(":open google ") end),
    ...
    key({"Control"}, "t",           function (w)    open_uri(globals.homepage) end),
    ...
    buf("^gH$",                     function (w, b, m) for i=1,m.count do open_uri(globals.homepage) end end, {count=1}),
    buf("^gh$",                     function (w)       w:navigate(globals.homepage) end),

    -- Open tab from current tab history
    buf("^gy$",                     function (w) open_uri(w.view.history or "") end),
